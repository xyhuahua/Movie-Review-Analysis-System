<template>
  <div style="width: 70%; margin-top: 5%" v-loading="isLoading">
    <el-form
      :model="form"
      label-width="80px"
      label-position="left"
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="_modifyInfo">确 定</el-button>
    </div>
  </div>
</template>
...
import { modifyInfo } from "@/js/api";
const movieSorts_type = [
  "剧情",
  "喜剧",
  "科幻",
  "悬疑",
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        address: [{ validator: check1, trigger: "blur" }],
        tel: [{ validator: check1, trigger: "blur" }],
      },
    };
  },
  created() {
	  ...
	  _getUser(){
				...
				getUser(userName).then((res) => {
					...
					for(var i = 0;i<res.like.length;i++){
						this.form.checkList.push(res.like[i].like) 
					}
